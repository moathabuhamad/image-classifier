...
def parse_args():
    ...
    parser.add_argument(
        "--filepath", dest="filepath", default="flowers/test/1/image_06743.jpg"
    )  # use a deafault filepath to a primrose image
    parser.add_argument(
        "--category_names", dest="category_names", default="cat_to_name.json"
    )
...
def process_image(image):
    ...
    if width > height:
        im.thumbnail(
            (5000000, 256)
        )  # used the arbitrary 5000000, but any large value will work
    else:
        im.thumbnail(
            (256, 5000000)
        )  # used the arbitrary 5000000, but any large value will work
...
def predict(image_path, model, topk):
    ...
    model.to(
        device
    )  # We should push the input data as well as the model onto the device
    ...
    torch_image_tensor.unsqueeze_(
        0
    )  # this needs to be added, to add a batch dimension to the input
    torch_image_tensor = torch_image_tensor.to(
        device
    )  # The input tensor should be on the GPU!!!
